...
import (
	...
	"github.com/vela-public/onekit/strkit"
	"io"
	"io/fs"
	"net/http"
	"os"
...
func FileExistsIn(file string, allowedPaths ...string) (string, error) {
	...
	uniqAllowedPaths := libkit.Dedupe(allowedPaths)
	...
	for _, allowedPath := range uniqAllowedPaths {
		...
		if strkit.HasPrefixAny(allowedAbsPath, ".") {
...
func ExecutableName() string {
	...
	return strkit.TrimSuffixAny(executableNameWithExt, filepath.Ext(executableNameWithExt))
}
...
func UseMusl(path string) (bool, error) {
	...
	for _, importedLibrary := range importedLibraries {
		if strkit.ContainsAny(importedLibrary, "libc.musl-") {
			return true, nil
		}
	}
	return false, nil
...
func HasPermission(fileName string, permission int) (bool, error) {
	...
	if err != nil {
		if os.IsPermission(err) {
			return false, fmt.Errorf("permission error %v", err)
		}
		return false, err
	}
	file.Close()
